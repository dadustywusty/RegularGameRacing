...
func tick(delta: float) -> void:
	...
	var forca_atual := ((aceleracao * mod_acc) * (velocidade * mod_vel)) + velocidade_turbo
	var velocidade_alvo := direcao_alvo * forca_atual
	
	if aceleracao != 0 or velocidade_turbo != 0:
		corpo.velocity = corpo.velocity.lerp(velocidade_alvo, (friccao * mod_fri) * delta)
	else:
		var nova_friccao = (friccao * mod_fri) if corpo.is_on_floor() else 0.5
