extends Node

@onready var loop: AudioStreamPlayer = $Loop

const MUSICA_MENU := preload("res://preciso de terapia/dusty folder/MUSGA/corrida menu.ogg")
const DELAY_ANTES_INTRO := 1.0

var _musica_atual: String = ""

func _ready() -> void:
	loop.stream = MUSICA_MENU
	loop.stream.loop = true
	loop.play()
	_musica_atual = "menu"

func tocar_menu() -> void:
	if _musica_atual == "menu":
		return
	loop.stop()
	_musica_atual = "menu"
	loop.stream = MUSICA_MENU
	loop.stream.loop = true
	loop.play()

func tocar_musica(stream: AudioStream, nome: String) -> void:
	if _musica_atual == nome:
		return
	loop.stop()
	_musica_atual = nome
	loop.stream = stream
	loop.stream.loop = true
	loop.play()

func tocar_musica_com_intro(intro_stream: AudioStream, loop_stream: AudioStream, nome: String) -> void:
	if _musica_atual == nome:
		return
	loop.stop()
	_musica_atual = nome
	await get_tree().create_timer(DELAY_ANTES_INTRO).timeout
	if _musica_atual != nome:
		return
	# Como não tem player de intro separado, toca a intro no mesmo loop
	loop.stream = intro_stream
	loop.stream.loop = false
	loop.play()
	await loop.finished
	if _musica_atual == nome:
		loop.stream = loop_stream
		loop.stream.loop = true
		loop.play()

func parar() -> void:
	loop.stop()
	_musica_atual = ""

func tocar_hover() -> void:
	pass

func tocar_click(acao: Callable) -> void:
	acao.call()
