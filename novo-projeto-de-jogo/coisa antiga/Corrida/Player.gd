...
func _physics_process(delta):
	...
	apply_friction()
	if is_grounded:
		ball.gravity_scale = gravity_multiplier * 0.5
	else:
		ball.gravity_scale = gravity_multiplier
...
func sync_car_position():
	...
	for i in range(ball.get_contact_count()):
		var normal = ball.get_contact_local_normal(i)
...
func apply_movement_force(delta):
	...
	var slope_forward = forward.slide(ground_normal).normalized()

	
	var slope_dot = clamp(ground_normal.dot(Vector3.UP), 0.0, 1.0)
	var slope_angle = acos(slope_dot)

	
	var engine_force = acceleration * 25.0


	var climb_boost = 1.0 + slope_angle * 8.0

	ball.apply_central_force(
		slope_forward * engine_force * input * climb_boost
	)
...
func apply_friction():
	...
	var flatness = clamp(ground_normal.dot(Vector3.UP), 0.0, 1.0)
	var friction_strength = friction_force * flatness * 0.4

	var friction = -ball.linear_velocity.normalized() * friction_strength
